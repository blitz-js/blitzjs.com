...
:::info
Unlike Next.js, your `api/` folder should be a sibling of `pages/`
instead of being nested inside. But `pages/api` is still supported for
...
```ts
const hello = (req: BlitzApiRequest, res: BlitzApiResponse) => {
  res.statusCode = 200
...
```ts
const example = (req: BlitzApiRequest, res: BlitzApiResponse) => {
  if (req.method === "POST") {
...
```ts
const postPid = (req: BlitzApiRequest, res: BlitzApiResponse) => {
  const {
    query: {pid},
  } = req
...
```ts
const slug = (req: BlitzApiRequest, res: BlitzApiResponse) => {
  const {
    query: {slug},
  } = req
