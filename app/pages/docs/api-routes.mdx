...
> API Routes [do not specify CORS headers](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS), meaning they are **same-origin only** by default. You can customize such behavior by wrapping the request handler with the [cors middleware](/docs/api-routes#connectexpress-middleware-support).
...
You can use the `getSessionContext` function to get the session of the user. Here is an example using session in a API route `app/api/customRoute.tsx`:
