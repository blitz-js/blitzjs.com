...
This can also be done globally for all pages by configuring `pageDefaults`
in `blitz-client.ts` in `AuthClientPlugin` constructor
...
AuthClientPlugin({
  ...
  pageDefaults: {
    authenticate: true,
    suppressFirstRenderFlicker: true
  },
}),
```
