...
file in the app root. This preset contians everything needed to compile
React applications and server-side code. The source code transformations
that enables
[the import and use](https://blitzjs.com/docs/client-and-server) of server
code directly in your UI components is also found here.

If you want to extend the default Babel configs, this is possible. The
babel.config.js at the top of your app is considered the source of truth,
and therefore it needs to define what Blitz needs as well, which is the
`blitz/babel` preset.
