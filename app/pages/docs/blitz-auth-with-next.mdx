...
- In the server-side configuration of Auth plugin, we need two additional
  ...
  - `storage` : Blitz stores session information in the database, so we
    need to specify how Blitz Auth can access the storage. We will use
    `PrismaStorage` and provide the database client, which we will create
    as the next step. If you’re interested in how it looks with something
    other than Prisma, you can check out this Redis example:
    https://gist.github.com/beerose/80f37b4b36cbd7ba2745701959e3cb8b.
...
to store the session information. The package is database agnostic, but
the simplest way to set it up is by using Prisma, and that’s what we’ll do
in this guide.
...
provides easy access to the Blitz’s `ctx` object, which contains
auth-related methods and properties.
