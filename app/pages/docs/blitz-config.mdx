...
Proxy support is enabled automatically for recipe install if `http_proxy`
environment variable is present. You can also set proxy option using:
...
Proxy configuration in `blitz.config.js` will override environment proxy
configuration.
