...
If you would like full control over the Blitz Logger, you may specify 
a `log.loggerFactory` function. This is useful if you would like to attach
custom transports to your logs, as in the example below.

The `loggerFactory` function receives an object with a `logger` 
and a `settings` object. The `logger` represents the default logger
that Blitz would normally use. The `settings` represents the settings that 
configure the logger that Blitz would normally use. The types for `logger` 
and `settings` come from the `tslog` package that Blitz uses for its logging operations.
You may find more information about `tslog` here: https://tslog.js.org
