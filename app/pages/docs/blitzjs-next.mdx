...
Blitz.js extends the `next.config.js` file by looking for the blitz property.
...
The Blitz CLI supports running custom Next.js servers. This means you can compile typescript and use the blitz cli to inject env variables. By default, the CLI checks for `server/index.ts` or `server.ts`
...
You can set the types returned from the Next.js data fetching functions. All Blitzjs wrappers for the Nextjs functions accept a generic. Same with the `BlitzPage` type.
