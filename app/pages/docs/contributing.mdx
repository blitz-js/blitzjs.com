...
Depending on your maintainer level, you will have the ability to make a
blitz release. The way it currently works, isn't set in stone so look here
for latest updates on how the release system works.
...
Essentially every user facing PR should have a changeset. When that PR
gets approved and merged to the `main` branch a new PR is either created
or modified called `Version packages (beta)`. This PR updates each
internal package's changelog & each blitz package's version by
incrementing it in the `package.json` files.
