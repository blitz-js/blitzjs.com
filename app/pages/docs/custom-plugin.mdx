...
sidebar_label: Blitz Client Plugins
---

### Overview {#overview}

Plugins can be used to extend the Blitz toolkit with custom functionality.
Plugins are defined using the `createClientPlugin` function from the
`blitz` package.

The plugin can then be used return the following:
...
- The `events` object contains the event hooks that the plugin could
  ...
  - The plugin can then perform any actions it needs to when the event is
    triggered.
...
- The `middleware` object contains the middleware hooks that the plugin
  ...
  - The plugin can then perform any actions it needs to when the
    middleware is called.
  - The code defined by each plugin `middleware` is executed in series
    through `pipes` so the output of one middleware is passed as the input
...
To create a client plugin, create a new file, for example in
...
To use there client plugin, import it in the `src/blitz-client` file and
add it to the `plugins` array as shown below:
