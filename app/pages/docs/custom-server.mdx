...
- `conf`: `object` - The same object you would use in [blitz.config.js](./blitz-config). Defaults to `{}`
...
> You may also wish to configure the client-side router to disallow client-side redirects to filename routes; for that refer to [`router.beforePopState`](./router#router-object).
