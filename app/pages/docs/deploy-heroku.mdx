...
2. In your package.json add a `start:production` and an `build`
   command. We'll call the start script in the `Procfile` below. And Heroku will run the `build` script for you on deploy.
