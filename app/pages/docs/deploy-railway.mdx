...
5. Change your build script in package.json to be `NODE_ENV=production
   blitz build && blitz prisma migrate deploy` so that the production DB
   will be migrated on each deploy
