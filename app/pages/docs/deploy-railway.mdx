...
1. Sign up for an account on [Railway](https://railway.app) - they have a free tier that is sufficient for small applications. 
...
2. Create a new project (via your [Dashboard](https://railway.app/dashboard) if not prompted)
...
3. If a database is required, select **Provision PostgreSQL** (or MongoDB/MySQL/Redis - whichever flavour your app requires). Once the database is provisioned, you will see the Project Setup page. 
...
Blitz requires the `SESSION_SECRET_KEY` variable to be set. Go to your project on the GUI, select the Variables link and enter `SESSION_SECRET_KEY` as the name of the variables a random 32-key secret as its value. Click **Add** to add the variable.
...
2. Go to the **Deployments** section. If your project is on GitHub, you can click on **Connect GitHub** to deploy and connect directly to your repositories on GitHub. Otherwise, you will have to use the CLI to deploy your application.

3. Select the repository and the branch to deploy from and click **Connect + Deploy**
...
4. Railway will now build and deploy your app on every push to this branch on your selected repository.
