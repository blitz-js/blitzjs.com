...
<Card type="info">
  Windows users must ensure the result of `yarn global bin` is in PATH in
  order to use `yarn` globally. To use `yarn` within Git Bash on Windows,
  ensure the result of `yarn global bin` is exported to PATH within
  `.bash_profile`.
</Card>
...
### Blitz Plugins {#blitz-plugins}

TODO

In the newly created app, you can see RPC plugin and Auth plugin set up by
default.

- sth that you can see blitz-server and blitz-client to see the setup
- in the next js project the setupBlitz functions are imported from
  blitzjs/next
- add sth that _here you can read more_
