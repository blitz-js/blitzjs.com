...
Note: Windows users must ensure the result of `yarn global bin` is in PATH in order to use `yarn` globally.
To use `yarn` within Git Bash on Windows, ensure the result of `yarn global bin` is exported to PATH within `.bash_profile`.
