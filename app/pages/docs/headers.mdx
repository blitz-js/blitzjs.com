...
### Headers with basePath support {#basepath}

When leveraging [`basePath` support](./blitz-config#base-path) with
headers each `source` is automatically prefixed with the `basePath` unless
you add `basePath: false` to the header:
...
### Headers with i18n support {#i18n}

When leveraging [`i18n` support](./i18n-routing) with headers each
...
[statically generated](./pages#automatic-static-optimization), you can do
so by setting `revalidate` in the page's
[`getStaticProps`](./get-static-props) function.
