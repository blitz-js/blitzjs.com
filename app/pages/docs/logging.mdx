...
The `logger` setup in `app/blitz-server.ts` is used by Blitz RPC to provide robust logging which can be setup and configured by following the options [here](https://blitzjs-com-git-siddhsuresh-blitz-rpc-verbose-blitzjs.vercel.app/docs/rpc-config#blitz-rpc-logging).
