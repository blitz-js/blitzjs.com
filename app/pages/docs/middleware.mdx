...
HTTP Middleware is the escape hatch for [queries](./query-resolvers) and [mutations](./mutation-resolvers) to access HTTP primitives. Middleware can then pass data to queries and mutations using the middleware `res.blitzCtx` parameter.
...
Global middleware runs for every Blitz query and mutation. It is defined in [`blitz.config.js`](./blitz-config) using the `middleware` key.
...
If you'd like more details on the RPC API, see the [RPC API documentation](./rpc-specification)
