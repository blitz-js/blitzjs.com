...
not a requirement!). If you're not doing it already, you can set up
...
Here's an example `tsconfig.json` that you can use:
...
Using [`basePath`](/blitz-config#base-path), you can configure your Blitz
application's assets and links to automatically work with your new subpath
`/store`. Since each page in Blitz is its own
...
URL of your domain. Then, you can use [`rewrites`](/rewrites) inside
