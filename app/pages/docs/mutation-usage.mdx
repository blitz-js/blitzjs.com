...
To use one of [your Blitz mutations](./query-resolvers), call the `useMutation` hook with your mutation resolver.
...
`useMutation` is a thin layer on top of the `useMutation` hook from [`react-query`](https://github.com/tannerlinsley/react-query).
...
For complete details, see the [`useMutation` documentation](./use-mutation).
...
- [`useQuery`](./use-query) returns a `setQueryData` function you can use to manually update the cache
...
[`useQuery`](./use-query) returns a `refetch` function you can use to trigger a query reload.
...
Another way, if you need to invalidate a query inside another component, is to use the [`invalidateQuery()`](./resolver-utilities#invalidateQuery) function provided by `blitz`.
