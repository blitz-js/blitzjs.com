...
1. Example: `example.com/api/auth/github?redirectUrl=${router.pathname}`

1. Via the config passed to `NextAuthAdapter`

1. If success, it will use `config.successRedirectUrl`
1. If error, it will use `config.errorRedirectUrl`
