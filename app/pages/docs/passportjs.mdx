...
You can access the middleware context by providing a callback to the
`passportAuth` adapter. The first argument of the callback is `ctx`. You can then access the session context via `ctx.session`
