...
- `ctx: Ctx`
  - This context object is automatically provided by Blitz at run time. This is used by [HTTP Middleware](./middleware) to provide data or functions to your query. For example, authentication middleware can provide `ctx.session` with all the data about the current user session.
