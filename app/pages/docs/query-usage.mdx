...
To use one of [your Blitz queries](./query-resolvers), call the `useQuery` hook with:
...
In `getServerSideProps`, pass a query function to [`invokeWithMiddleware`](./resolver-server-utilities#invoke-with-middleware) which will ensure appropriate middleware is run before/after your query function.
