...
> **Note**: if you want to prefetch an infinite query, you'll have to use
> `prefetchInfiniteQuery` instead of `prefetchQuery` and
> [`getInfiniteQueryKey`](/docs/resolver-client-utilities#get-infinite-query-key)
> instead of `getQueryKey`.
...
export const getServerSideProps = async ({ params, req, res }) => {
  const project = await invokeWithMiddleware(
    getProject,
