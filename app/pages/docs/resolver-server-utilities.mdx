...
Blitz exports a `resolver` object which contains a few utilities. "Resolver" as used here and for [queries](./query-resolvers) and [mutations](./mutation-resolvers) refers to a function that takes some input and "resolves" that into some output or side effect. This term is commonly used for GraphQL. A resolver is separate from an API endpoint, because in an API endpoint you have full access to the Node request and response objects and you are responsible for managing the http stuff. But resolvers are a layer removed from raw API handlers.

The below utilities are exported under the `resolver` object instead of `resolverPipe` and `resolverZod` since they are commonly used together. This cleans up your imports and makes it easy to discover any other utilites on the `resolver` object.
...
This is a handly utility for using [Zod](https://github.com/colinhacks/zod),
