...
This hook returns all the parameters for the current route as an object.
The default parameter type is `undefined | string | string[]`, but you can
optionally provide a type argument to narrow the parameters and types that
are returned.
...
This hook return a single parameters for the current route. The default
return type is `undefined | string | string[]`, but you can optionally
provide a type argument to cast the return type.
...
Unlike `useParams`, this hook will parse all values to their respective
types. For example:
...
// ?yea=true&nay=false
query = {yea: true, nay: false}

// ?include=clients&include=products
query = {include: ["clients", "products"]}

// ?all-types=str&all-types=1.2&all-types=false
query = {"all-types": ["str", 1.2, false]}
```
