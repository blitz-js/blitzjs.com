...
[`headers`](./headers) in `blitz.config.js` to apply HTTP response headers
...
and more. This prefetching is performed in the background, so
the [DNS](https://developer.mozilla.org/en-US/docs/Glossary/DNS) is more
likely to be resolved by the time the referenced items are needed. This
...
[Vercel](https://vercel.com/docs/edge-network/headers#strict-transport-security),
...
[`Content-Security-Policy`](#content-security-policy) disabling the use of
...
[here](https://www.w3.org/TR/permissions-policy-1/).
...
treated as a different `Content-Type` like an executable, which could be
...
[here](https://scotthelme.co.uk/a-new-security-header-referrer-policy/).
