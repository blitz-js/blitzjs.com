...
[accessible anywhere on the server](#session-on-the-server).
...
API routes with `getSessionContext` like this:
...
[the default config that works with Prisma](https://github.com/blitz-js/blitz/blob/canary/packages/server/src/supertokens.ts#L62-L85).
