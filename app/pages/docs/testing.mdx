...
global providers, but also testing certain parts of your app which Blitz
handles internally such as routing and data querying.
...
-`client` - The global query client used by React Query. The default
`queryClient` exported from `blitz` can be used here.

-`contextSharing` - Controls whether or not the provider context can be
shared across multiple applications. This can be useful when working with
micro-frontends. Defaults to `false`.
