...
Blitz's level of lock-in is one level higher than Next.js. A Blitz codebase is not a *lot* different than a Next.js codebase. But it is enough different that it wouldn't be super easy to "eject" to plain Next.js. For example, you'll probably use the Blitz authentication which is unique to Blitz. But switching to Next Auth would give you a comparable experience with Next.js.
...
But if you use yarn, you can use the `resolutions` field in package.json to upgrade next.js before we do.


## Multi-Client API


Some folks say, "you'll eventually need GraphQL so you should just start with it". While we believe most apps won't grow large enough to warrant GraphQL, the question of an API for multiple clients is an excellent point.


We fully acknowledge that many web apps will need a mobile app sooner or later. And we want to make this integration as easy as possible!
...
- You can directly access the auto-generated API endpoints and you can use them in any mobile app (we currently have a few people doing this)
- You can see all generated endpoints in your app using [`blitz routes`](./cli-routes) command
- You can see the RPC specification for the endpoints here: [RPC Specification](./rpc-specification)

**What we plan to do going forward:**

1. First step, add docs and tools for making it easier to manually use the generated API endpoints. See https://github.com/blitz-js/blitz/discussions/1907 and https://github.com/blitz-js/blitz/issues/745
2. Next, add ability generate a JS/TS client library for easily using the generated endpoints from an app
3. Next, add some sort of first-class react-native support where you can import blitz queries/mutations directly into your react-native code and have it magically work just like it does today with your web app. After the 1.0 release, this will be Brandon's main focus for the year.
...
Running multiple background processes isn't super difficult, but we want to add docs and APIs that make this as easy as possible.
