...
And then run `blitz prisma generate` to update prisma client for that schema change. This doesn't require a database migration because there's not an actual field being added to the `Question` model in the database.
...
If you click the **Edit** button on one of your existing questions, you'll see it uses the same form as creating questions. So that part is already done! We just need to update our mutation.
