...
Note that this doesn't require a database migration because we haven't added the 
`Choice` field to the `Question` model yet. So we are choosing `false` when 
prompted to run the migration:
...
Now our database is ready and a Prisma client is also generated. Lets move on to
play with the Prisma client!
