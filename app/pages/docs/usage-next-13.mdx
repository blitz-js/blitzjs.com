...
Blitz requires **only a breaking change** with the major upgrade of `Next 12` ➔ `Next 13` for usage with the `pages` directory. You can even continue to use the old blitz layout with all your files in `app`. But you'll need to rename that directory if you want to use the new Next.js app router.
...
invoke method now returns a promise of the resolvers.

**Change to be made**

```diff
+ const user = await invoke(getCurrentUser, null)
- const user = invoke(getCurrentUser, null)
```
