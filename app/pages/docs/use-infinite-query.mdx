...
function Projects(props) {
  ...
  ] = useInfiniteQuery(getProjects, (page = {take: 3, skip: 0}) => page, {
    getNextPageParam: (lastPage) => lastPage.nextPage,
  })
  return (
    <>
      {projectPages.map((group, i) => (
...
- `getNextPageParam: (lastPage, allPages) => unknown | undefined`
  ...
  - It should return a **single variable** that will be passed as the last
    optional parameter to `getQueryInputArguments()`.
...
- `getPreviousPageParam: (firstPage, allPages) => unknown | undefined`
  - When new data is received for this query, this function receives both
    ...
    pages.
  - It should return a **single variable** that will be passed as the last
    optional parameter to your `getQueryInputArguments()`.
  - Return `undefined` to indicate there is no previous page available.

### Returns {#returns}

`[infiniteQueryResult, queryExtras]`
