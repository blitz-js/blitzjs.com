...
The useQuery hook can also be used for paginated queries. Achieving
pagination can be as simple as specifying the `skip` and `take` query
input arguments:

```ts
import {Suspense} from "react"
import {Link, BlitzPage, useQuery, useRouterQuery, useRouter} from "blitz"
...
const Projects = () => {
  ...
  const [{projects, hasMore}] = useQuery(getProjects, {
...
type GetProjectsInput = {
  where?: Prisma.ProjectFindManyArgs["where"]
...
export default async function getProjects(
  {where, orderBy, take, skip}: GetProjectsInput,
...
While this approach is fine, you may notice loading indicators when
navigating between pages. This behavior is expected, since updating the
page index triggers a new query. By setting `keepPreviousData: true` in
the query options and tracking `isPreviousData`, you can improve user
experience by continuing to render the current data until the next page of
data is available.

In the updated example below, a user navigates to the next page,
triggering a query for new data. The previous data continues to render
until the query resolves, at which point the previous results are swapped
out with the new ones, bypassing the interim loading state entirely.
...
const Projects = () => {
  const router = useRouter()
  const {page = 0} = useRouterQuery()
  const [{projects, hasMore}, {isPreviousData}] = useQuery(
    getProjects,
    {
      skip: ITEMS_PER_PAGE * Number(page),
      take: ITEMS_PER_PAGE,
