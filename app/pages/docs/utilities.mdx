...
A utility function that wraps middleware to adapt and work within the `blitz-server` setup file as a plugin.
...
A `BlitzServerPlugin` that can be initialized inside the `blitz-server` setup file.