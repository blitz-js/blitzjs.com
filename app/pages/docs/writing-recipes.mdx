...
lets you modify it. It takes the `program` and a callback. The callback
has the config `j.ObjectExpression` and has to return another
`j.ObjectExpression`.
...
plugin/preset. You don't need to worry about jscodeshift stuff, just
provide a valid JSON object and it will be transformed. This argument has
to be either the name of the plugin/preset (as a string) or a array with
two elements: the name of the plugin/preset and its configuration.
