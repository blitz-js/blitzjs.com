...
Some recipes may require modifying the existing Blitz config file.

#### Create a tansformation step

We are fortunate enough to be able to use the `paths` provided by the
...
#### Adding middleware {#adding-blitz-middleware}

Using the transform function `addBlitzMiddleware` you can very easily add
new middleware from a recipe.
      ...
      j.blockStatement([
        j.expressionStatement(
          j.assignmentExpression(
            "=",
            j.memberExpression(j.identifier("res"), j.identifier("blitzCtx.foo")),
            j.identifier("bar"),
          ),
        ),
