...
Any file inside an `api/` folder are accessible at a URL corresponding to its path inside `api/`. So `app/modules/projects/api/webhook.ts` will be at `localhost:3000/api/webhook`.
