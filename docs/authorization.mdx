...
##### `ctx.session.$isAuthorized(roleOrRoles?: string | string[], options?: {if?: boolean})`
...
##### `ctx.session.$authorize(roleOrRoles?: string | string[], options?: {if?: boolean})`
...
ctx.session.$authorize("admin", {if: inputUserId !== currentUserId}) // throws only if condition true
