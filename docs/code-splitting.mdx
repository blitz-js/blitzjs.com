...
Blitz automatically optimizes your bundles and splits them up by page, so your users will only download the required assets for a given route (without an extra effort on your part!). This technique is known as [code splitting](https://reactjs.org/docs/code-splitting.html).

In the event you need to split or lazy-load assets within a page, Blitz supports ES2020 [dynamic `import()`](https://github.com/tc39/proposal-dynamic-import) for JavaScript. With it you can import JavaScript modules (React components, libraries, etc.) dynamically and work with them. You can think of dynamic imports as another way to split your code into manageable chunks.
