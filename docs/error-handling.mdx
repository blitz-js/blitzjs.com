...
To use, import from `blitz` and use like any JavaScript Error. If you're curious, you can [see the source code for these.](TODO)

```ts
import {AuthenticationError, isAuthenticationError} from "blitz"
...
By default, new Blitz applications come with [`react-error-boundary`](https://github.com/bvaughn/react-error-boundary) installed with a top-level `ErrorBoundary` and `FallbackComponent` in `app/pages/_app.tsx`.
...
For example, with the above `_app.tsx`, you can throw `AuthenticationError` inside a Blitz query and then a login screen will automatically show in the client because that root ErrorBoundary is rendering `<LoginForm>` if `isAuthenticationError(error)` returns `true`.
...
See the [Error Pages documentation](./error-pages) to learn about custom 404 and 500 error pages.
