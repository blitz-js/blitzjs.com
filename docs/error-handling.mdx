...
To make this typesave and get autocomplete in editor you can use TS TypeGuard

```ts
// typeguard
function isError<T extends new (...args: any) => any>(err: any, name: T): err is InstanceType<T> {
  if (name == err.name) {
    return true
  }
...
try {
  ...
  if (isError(err, UsernameTakenError)) {
    // TS can now infer that err is UsernameTakenError
    console.log(err.suggestedUserName)
  }
}
```
