...
- The file structure nested inside `app` can be anything you want, but we recommend the following to keep all "container" directories inside `app/modules` folder.
- Typically you will have two types of "container" directories inside `app/modules` that can be nested or combined any way you want:
  - **entity** directories like `modules/projects/` and `modules/tasks/`
  - **context** directories like `modules/marketing/` or `modules/admin/`.
