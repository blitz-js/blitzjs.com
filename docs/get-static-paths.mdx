...
The `paths` key determines which paths will be pre-rendered. For example, suppose that you have a page that uses dynamic routes named `app/modules/posts/pages/posts/[id].js`. If you export `getStaticPaths` from this page and return the following for `paths`:
...
Then Blitz will statically generate `posts/1` and `posts/2` at build time using the page component in `app/modules/posts/pages/posts/[id].js`.

Note that the value for each `params` must match the parameters used in the page name:

- If the page name is `app/modules/posts/pages/posts/[postId]/[commentId]`, then `params` should contain `postId` and `commentId`.
...
Here’s an example which pre-renders one blog post per page called `app/modules/posts/pages/posts/[id].js`. The list of blog posts will be fetched from your database or a CMS and returned by `getStaticPaths` . Then, for each page, it fetches the data for a single post using `getStaticProps`.

```jsx
// app/modules/posts/pages/posts/[id].js
