...
- Default: Works automatically with `blitz start`, or a custom server
...
When using an external URL, you must add it to [domains](#domains) in `blitz.config.js`.
...
A custom function used to resolve URLs. Defaults to [`images` object in `blitz.config.js`](/docs/basic-features/image-optimization.md#loader).
