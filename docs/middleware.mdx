...
2. You can pass a callback into the resolver via `res.blitzCtx` and then the resolver can pass data back to the middlewary be calling `ctx.someCallback(someData)`
