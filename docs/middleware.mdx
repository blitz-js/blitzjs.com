...
HTTP Middleware is the escape hatch for [queries](./query-resolvers.mdx) and [mutations](./mutation-resolvers.mdx) to access HTTP primitives. Middleware can then pass data to queries and mutations using the middleware `res.blitzCtx` parameter.
...
```ts
// app/modules/products/queries/getProduct.tsx
