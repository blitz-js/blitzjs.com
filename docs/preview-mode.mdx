...
In the [Pages documentation](./pages), we talked about how to pre-render a page at build time (**Static Generation**) using [`getStaticProps`](./get-static-props) and [`getStaticPaths`](./get-static-paths).

Static Generation is useful when your pages fetch data from a headless CMS. However, it’s not ideal when you’re writing a draft on your headless CMS and want to **preview** the draft immediately on your page. You’d want Blitz to render these pages at **request time** instead of build time and fetch the draft content instead of the published content. You’d want Blitz to bypass Static Generation only for this specific case.

Blitz has the feature called **Preview Mode** which solves this problem. Here’s an instruction on how to use it.

## Step 1. Create and access a preview API route

> Take a look at the [API Routes documentation](./custom-api-routes.mdx) first if you’re not familiar with Blitz API Routes.

First, create a **preview API route**. It can have any name - e.g. `app/api/preview.js` (or `.ts` if using TypeScript).
...
`res.setPreviewData` sets some **cookies** on the browser which turns on the preview mode. Any requests to Blitz containing these cookies will be considered as the **preview mode**, and the behavior for statically generated pages will change (more on this later).
...
**First**, you should create a **secret token string** using a token generator of your choice. This secret will only be known by your Blitz app and your headless CMS. This secret prevents people who don’t have access to your CMS from accessing preview URLs.

**Second**, if your headless CMS supports setting custom preview URLs, specify the following as the preview URL. (This assumes that your preview API route is located at `app/api/preview.js`.)
...
### Unique per `blitz build`

The bypass cookie value and private key for encrypting the `previewData` changes when a `blitz build` is ran, this ensures that the bypass cookie can’t be guessed.
