...
- Queries that fail in production will silently and automatically be retried **2 times, with exponential backoff delay** before capturing and displaying an error to the UI. To change this, you can alter the default `retry` and `retryDelay` options for queries to something other than `2` and the default exponential backoff function.
  - By default, queries with the following errors will not be retried:
    - `AuthenticationError`
    - `AuthorizationError`
