...
## `resolver.authorizeIf`

This is like `resolver.authorizeIf` but the first argument is a callback function that determines whether the authorization will be enforced or not
...
export default resolver.pipe(
  resolver.authorize(),
// highlight-start
  resolver.authorizeIf((input, ctx) => input.orgId !== ctx.session.orgId, "admin"),
// highlight-end
  async (input, ctx) => {
    // stuff
  },
...
<!-- prettier-ignore -->
```js
const pipeFn = resolver.authorizeIf((input, ctx: Ctx) => boolean, ...isAuthorizedArgs)
```
