...
Blitz uses a file-system based router that's built on [Next.js](https://nextjs.org/docs/routing/introduction).

- All components in `pages/` are mapped to a URL. ([Pages documentation](./pages))
- All http handlers in `api/` are mapped to a URL. ([API routes documentation](./api-routes.mdx))
- Queries and mutations are automatically exposed as an API endpoint
  - The `app/modules/projects/queries/getProjects.js` query is exposed at `/api/projects/queries/getProjects`
...
Any route like `/post/1`, `/post/abc`, etc. will be matched by `app/pages/post/[pid].js`. The matched path parameter will be sent as a query parameter to the page, and it will be merged with the other query parameters.
...
Multiple dynamic route segments work the same way. The page `app/pages/post/[pid]/[comment].js` will match the route `/post/abc/a-comment` and its `query` object will be:
...
For example, `app/pages/post/[[...slug]].js` will match `/post`, `/post/a`, `/post/a/b`, and so on.
...
- Pages that are statically optimized by [Automatic Static Optimization](./pages#automatic-static-optimization) will be hydrated without their route parameters provided, i.e `query` will be an empty object (`{}`).
