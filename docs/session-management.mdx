...
If you are using [`getServerSideProps`](./get-server-side-props), then you can pass the session public data to `useSession()` with the `initialPublicData` option.
...
export const getServerSideProps: GetServerSideProps = async ({req, res}) => {
  ...
  return {props: {publicData: initialPublicData}}
}

const SomeComponent: BlitzPage = ({initialPublicData}) => {
