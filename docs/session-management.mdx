...
You login, logout, and otherwise modify the session via the [`SessionContext`](#sessioncontext) object which is [accessible anywhere on the server](#session-on-the-server).
...
export default async function login(input: SomeTSInputType, ctx: Ctx) {
  ...
  await ctx.session.create({userId: user.id, roles: [user.role]}) // highlight-line
}
```
...
Each session has [`PublicData`](#publicdata), data which is available on the client and has the potential to be read by third-parties because it's stored in a cookie readable by any Javascript code. This is usually used to store the current user id, user role, and perhaps current organization id.
...
If you want to avoid waiting for the session to load you can get it from [`getServerSideProps`](#in-getserversideprops-or-api-routes) and pass the public data as parameter to `useSession()`
...
const SomeComponent: BlitzPage = ({publicData}) => {
  const session = useSession({initialPublicData: publicData})
