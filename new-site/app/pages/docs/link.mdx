...
- `prefetch` - Prefetch the page in the background. Defaults to `true`. Any `<Link />` that is in the viewport (initially or through scroll) will be preloaded. Pages using [Static Generation](./get-static-props.mdx) will preload `JSON` files with the data for faster page transitions.
