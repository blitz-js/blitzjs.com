...
Save the file and you should see the page update in your browser. You can add customize this as much as you want. When you’re ready, move on to the next section.
...
Note that when starting your first real project, you may want to use a more scalable database like PostgreSQL, to avoid the pains of switching your database down the road. For more information, see [Database overview](database-overview.mdx). For now, we will continue with the default SQLite database.

## Scaffolding code for our models

Blitz provides a handy CLI command called [`generate`](./cli-generate) for scaffolding out boilerplate code. We'll use `generate` to create two models: `Question` and `Choice`. A `Question` has the text of the question and a list of choices. A `Choice` has the text of the choice, a vote count, and an associated question. Blitz will automatically generate an id, a creation timestamp, and a last updated timestamp for both models.
...
```
Again, hit **Enter** when prompted to run the migration and enter a name for the migration.
...
Then open `app/questions/mutations/createQuestionMutation.ts` and update the zod schema so that the choice data is accepted in the mutation. And we also need to update the `db.question.create` call so that the choices will also be created.


<!-- prettier-ignore -->
```diff
//app/questions/mutations/createQuestionMutation.ts
...
## Let’s do one more thing - let people vote on these questions!
...
Then, import the `updateChoice` mutation we just updated and create a `handleVote` function in our page:
